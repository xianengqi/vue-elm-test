<template>
  <div class="food_container">
       ...
       <!-- 排序 -->
            ...
            <section v-show="sortBy == 'sort'" class="sort_detail_type">
              <ul class="sort_list_container" @click="sortList($event)">
                <li class="sort_list_li">
                  <svg>
                    <use xmlns:xlink="http://www.w3.org/1999/xlink"  xlink:href="#icon-order-amount"></use>
                  </svg>
                  <p data="0" :class="{sort_select: sortByType == 0}">
                    <span>智能排序</span>
                    <svg v-if="sortByType == 0">
                      <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-gouxuan"></use>
                    </svg>
                  </p>
                </li>
                <li class="sort_list_li">
                  <svg>
                    <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-zuobiao"></use>
                  </svg>
                ...
                <li class="sort_list_li">
                  <svg>
                    <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-hot"></use>
                  </svg>
                ...
                <li class="sort_list_li">
                  <svg>
                    <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-jiage"></use>
                  </svg>
                ...
                <li class="sort_list_li">
                  <svg>
                    <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-tubiaokuwenjian-"></use>
                  </svg>
                ...
                <li class="sort_list_li">
                  <svg>
                    <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-pingfen"></use>
                  </svg>
        ...
        <div class="sort_item" :class="{choose_type:sortBy == 'activity'}">
          <div class="sort_item_container" @click="chooseType('activity')">
            <span :class="{category_title: sortBy == 'activity'}">率选</span>
            <svg width="10" height="10" xmlns="http://www.w3.org/2000/svg" version="1.1" class="sort_icon">
              <polygon points="0,3 10,3 5,8"/>
            </svg>
          </div>
              ...
              <section style="width: 100%">
                ...
                <ul class="filter_ul">
                  <li v-for="(item, index) in Delivery" :key="index" class="filter_li" @click="selectDeliveryMode(item.id)">
                    <svg class="iconFeng" :style="{opacity: (item.id == 0) && (delivery_mode !== 0) ? 0 : 1}">
                      <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="delivery_mode == item.id ? '#icon-gouxuan' : '#icon-fengniao1' "></use>
                    </svg>
    ...
    <section class="shop_list_container">
      <shop-list :geohash="geohash" :restaurantCategoryId="restaurant_category_id" :restaurantCategoryIds="restaurant_category_ids" :sortByType="sortByType" v-if="latitude" :confirmSelect="confirmStatus" :supportIds="support_ids"></shop-list>
    </section>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async initData () {
      ...
      // 记录support_ids 的状态，默认不选中，点击状态取反， status为true时为选中状态
    ...
    // 选中Category右侧列表的某个选项时，进行率选，重新获取数据并渲染
    ...
    selectDeliveryMode (id) {
      // delivery_mode 为空时，选中当前项，并且filterNum加1
      if (this.delivery_mode == null) {
        this.filterNum++
        this.delivery_mode = id
        // delivery_mode为当前已有值时，清空所选项，并且filterNum减1
      } else if (this.delivery_mode === id) {
        this.filterNum--
        this.delivery_mode = null
        // delivery_mode 已有值且不等于当前选择值，则赋值delivery_mode为当前所选id
      } else {
        this.delivery_mode = id
      }
    },
    ...
    // 点击确认时， 将需要率选的id值传递给子组件，并且收回列表
    confirmSelectFun () {
      // 状态改变时，因为子组件进行了监听，会重新获取数据进行率选
